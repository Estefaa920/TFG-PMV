...
import com.cine.app.service.CompraService;
...
/**
 * REST controller for managing {@link com.cine.app.domain.Compra}.
 */
@RestController
@RequestMapping("/api")
public class CompraResource {
    ...
    public CompraResource(CompraService compraService, CompraRepository compraRepository, ButacaService butacaService, PedidoService pedidoService, AforoService aforoService) {
    ...
    public ResponseEntity<Compra> createCompra(@Valid @RequestBody Compra compra) throws URISyntaxException {
        ...
        log.debug("ANTES DE EJECUTAR LA COMPRA");

        Compra result = compraService.save(compra);

        if(result!=null){
            aforoService.reservarbutacas(result.getAforos());
            pedidoService.guardarPedido(result.getPedidos());         
        }
     ...
     */
    @GetMapping("/compras")
    public ResponseEntity<List<Compra>> getAllCompras(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
